...
-- Treat manual "Opening Due" (পুরাতন বকেয়া) rows exactly like account-adjustment rows:
-- they COUNT toward the vendor's payable due, but are NEVER absorbed by advance/FIFO.
-- This lets a real pre-system old-due be recorded transparently instead of silently
-- letting "old due" deposits leak into current service bills (root cause of the
-- recurring "due shows short" bug).
...
BEGIN
  ...
  -- Opening / old-due / account-adjustment entries must never absorb advance.
  IF COALESCE(NEW.service_type, '') IN ('Account Adjustment','Opening Due') THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  -- Apply advance ONLY to eligible bills: received from vendor, and never to
  -- opening / account-adjustment rows. Keeps advance distribution in lock-step
  -- with get_vendor_balances.
  FOR v_bill IN
