-- Add a per-extra-service note that flows into the mirrored ledger entries
ALTER TABLE public.extra_services ADD COLUMN IF NOT EXISTS notes text;

-- Recreate the sync trigger so the note is written to vendor & agency ledgers
