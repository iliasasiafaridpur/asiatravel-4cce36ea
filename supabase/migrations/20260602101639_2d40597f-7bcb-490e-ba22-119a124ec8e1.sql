CREATE OR REPLACE FUNCTION public.next_module_id(_prefix text, _table text, _column text, _entry_date date DEFAULT NULL)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  full_prefix := _prefix || '-' || to_char(base_date, 'YYMM') || '-';
  q := format(
