-- 1) Track receipt/discount directly on each extra service row (source of truth,
--    works for both agency-billed and "Self" passengers).
...
-- 2) Backfill received/discount from any existing customer-ledger mirror so
--    nothing already collected is lost.
UPDATE public.extra_services e
SET received_amount = COALESCE(l.received_amount, 0),
    discount_amount = COALESCE(l.discount_amount, 0)
FROM public.agency_ledger l
WHERE l.source_table = 'extra_services'
  AND l.source_id = e.id
  AND (COALESCE(l.received_amount,0) > 0 OR COALESCE(l.discount_amount,0) > 0);

-- 3) Update the sync trigger so the agency_ledger mirror reflects the receipt /
--    discount stored on the extra service (keeps customer ledger in sync).
