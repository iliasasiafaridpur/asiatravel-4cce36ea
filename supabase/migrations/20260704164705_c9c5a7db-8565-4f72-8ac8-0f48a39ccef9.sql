...
-- Repair the already-deleted Jahangir QA / Rubel Sheikh ৳20,000 receipt impact.
UPDATE public.tickets t
   SET received = 6000,
       payment_date = DATE '2026-06-21',
       updated_at = now()
 WHERE t.id = '67d78c92-bb78-4b56-95d5-cfc23bd9e275'::uuid
   AND COALESCE(t.received, 0) = 26000
   AND EXISTS (
     SELECT 1
       FROM public.agency_ledger al
      WHERE al.source_table = 'tickets'
        AND al.source_id = t.id
        AND lower(btrim(al.agent_name)) = lower('Jahangir QA')
        AND al.id = '51f41f86-6eab-42ea-8566-88fb57cb8fd5'::uuid
   );