...
BEGIN
  ...
  -- If the underlying service already has its own receipt row (Due Receive,
  -- service-form receipt, status-change receipt, etc.), do NOT mirror the
  -- auto agency-ledger row again. That would double count staff cash.
  IF v_source = 'agency_ledger_payment' AND EXISTS (
    SELECT 1
      FROM public.payment_receipts pr
     WHERE pr.service_table = NEW.source_table
       AND pr.service_row_id = NEW.source_id
       AND pr.source <> 'discount'
...
WHERE pr.service_table = 'agency_ledger'
  ...
  AND EXISTS (
    SELECT 1
      FROM public.payment_receipts direct_pr
     WHERE direct_pr.service_table = al.source_table
       AND direct_pr.service_row_id = al.source_id
       AND direct_pr.source <> 'discount'
