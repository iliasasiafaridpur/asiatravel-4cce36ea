  ...
  const handlePrint = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>Receipt ${receipt.receiptId}</title>
      <style>
        ...
        body::before { content:""; position:fixed; inset:0; z-index:9999; pointer-events:none; background-image:url("${window.location.origin}${logoAsset.url}"); background-repeat:no-repeat; background-position:center; background-size:70%; opacity:0.06; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
