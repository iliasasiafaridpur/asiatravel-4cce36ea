...
import { Download, Plus, Wallet, ArrowDownLeft, ArrowUpRight, Receipt, Users as UsersIcon } from "lucide-react";
import { toast } from "sonner";

// ===== Types =====
export interface AcctUser { id: string; name: string; phone: string; currentBalance: number; }
...
export default function AccountingModule() {
  ...
  const totals = useMemo(() => {
    let received = 0, expense = 0, handover = 0, handoverToday = 0;
    const t = today();
    for (const x of txs) {
    ...
    const totalReceivable = users.reduce((s, u) => s + (u.currentBalance > 0 ? u.currentBalance : 0), 0);
  ...
  const adjustUserBalance = (userId: string, delta: number) => {
    setUsers((prev) => prev.map((u) => u.id === userId ? { ...u, currentBalance: u.currentBalance + delta } : u));
  };

  const recordReceived = (userId: string, amount: number, category: string, description: string, date: string) => {
    if (!userId) return toast.error("User select করুন");
    if (!(amount > 0)) return toast.error("Amount দিন");
    const tx: Transaction = { id: uid(), userId, type: "received", amount, category, date, description };
    setTxs((p) => [tx, ...p]);
  ...
  const recordExpense = (amount: number, category: string, description: string, date: string) => {
    if (!(amount > 0)) return toast.error("Amount দিন");
    const tx: Transaction = { id: uid(), userId: null, type: "expense", amount, category, date, description };
    setTxs((p) => [tx, ...p]);
  ...
  const recordHandover = (userId: string, amount: number, description: string, date: string) => {
    ...
    if (!u) return;
    if (!(amount > 0)) return toast.error("Amount দিন");
    if (amount > u.currentBalance) return toast.error(`Handover (${fmt(amount)}) > Balance (${fmt(u.currentBalance)})`);
    const tx: Transaction = { id: uid(), userId, type: "handover", amount, category: "Hand Over", date, description };
    setTxs((p) => [tx, ...p]);
  ...
  return (
    <div className="relative z-10 space-y-4 max-w-6xl mx-auto p-2 sm:p-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <SummaryCard label="Cash in Hand" value={totals.cashInHand} icon={<Wallet className="h-4 w-4" />} highlight />
        <SummaryCard label="Total Receivable" value={totals.totalReceivable} icon={<ArrowDownLeft className="h-4 w-4" />} />
        <SummaryCard label="Today's Handover" value={totals.handoverToday} icon={<ArrowUpRight className="h-4 w-4" />} />
        <SummaryCard label="Total Expense" value={totals.expense} icon={<Receipt className="h-4 w-4" />} />
      </div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <Button onClick={() => { onAdd(name, phone); setName(""); setPhone(""); setOpen(false); }} className="w-full">Save</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
...
// ===== Tab 1: Receive =====
function ReceiveTab({ users, onAddUser, onSubmit }: {
  users: AcctUser[]; onAddUser: (n: string, p: string) => void;
  onSubmit: (userId: string, amount: number, category: string, description: string, date: string) => void;
}) {
      ...
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <Select value={userId} onValueChange={setUserId}>
            ...
            <SelectContent>
              {users.length === 0 ? <div className="px-3 py-2 text-sm text-muted-foreground">কোনো user নেই — উপরে New User দিন</div>
                : users.map((u) => <SelectItem key={u.id} value={u.id}>{u.name} {u.phone ? `(${u.phone})` : ""}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div>
...
// ===== Tab 2: Expense =====
function ExpenseTab({ onSubmit }: { onSubmit: (amount: number, category: string, description: string, date: string) => void }) {
  const [amount, setAmount] = useState(0);
  ...
  return (
    <Card>
      <CardHeader className="pb-2"><CardTitle className="text-base">Expense Entry (Others Khata)</CardTitle></CardHeader>
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <Label>Category</Label>
          <Select value={category} onValueChange={setCategory}>
...
// ===== Tab 3: Handover =====
function HandoverTab({ users, onSubmit }: {
  users: AcctUser[];
  ...
  return (
    <Card>
      <CardHeader className="pb-2"><CardTitle className="text-base">Cash Hand-over (To Authority/Agent)</CardTitle></CardHeader>
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <Label>User</Label>
          <Select value={userId} onValueChange={setUserId}>
            <SelectTrigger><SelectValue placeholder="Select user" /></SelectTrigger>
            <SelectContent>
              {users.map((u) => <SelectItem key={u.id} value={u.id}>{u.name} — {fmt(u.currentBalance)}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div>
...
function LedgerTab({ users, txs }: { users: AcctUser[]; txs: Transaction[] }) {
  ...
  const exportCsv = () => {
    ...
    const rows = withRunning.map((t) => [
      t.date, t.userId ? userMap.get(t.userId) ?? "" : "—",
      t.type, t.category, t.amount, t.running, t.description.replaceAll("\n", " "),
    ]);
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          ...
          <div className="flex items-end"><Button variant="outline" onClick={exportCsv} className="w-full gap-1.5"><Download className="h-4 w-4" />Export CSV</Button></div>
        </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Running Cash</TableHead><TableHead>Description</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {withRunning.length === 0 ? (
                ...
                <TableRow key={t.id} className={`row-tint-${idx % 4}`}>
                  ...
                  <TableCell>
                    <Badge variant={t.type === "received" ? "default" : t.type === "expense" ? "destructive" : "secondary"}>{t.type}</Badge>
                  </TableCell>
...
// ===== Tab 5: Balances =====
function BalancesTab({ users, txs, onAddUser }: { users: AcctUser[]; txs: Transaction[]; onAddUser: (n: string, p: string) => void }) {
  const [openUser, setOpenUser] = useState<AcctUser | null>(null);
  const userTxs = useMemo(() => openUser ? txs.filter((t) => t.userId === openUser.id) : [], [openUser, txs]);
  return (
    <Card>
      <CardHeader className="pb-2 flex-row items-center justify-between">
        <CardTitle className="text-base flex items-center gap-2"><UsersIcon className="h-4 w-4" /> All Users Balance</CardTitle>
        <AddUserDialog onAdd={onAddUser} />
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto rounded-md border">
          <Table>
            <TableHeader>
              <TableRow><TableHead>Name</TableHead><TableHead>Phone</TableHead><TableHead className="text-right">Current Balance</TableHead><TableHead>Status</TableHead></TableRow>
            </TableHeader>
            <TableBody>
              {users.length === 0 ? (
                <TableRow><TableCell colSpan={4} className="text-center py-6 text-muted-foreground">কোনো user নেই</TableCell></TableRow>
              ) : users.map((u, idx) => (
                <TableRow key={u.id} className={`cursor-pointer row-tint-${idx % 4}`} onClick={() => setOpenUser(u)}>
                  ...
                  <TableCell>
                    {u.currentBalance > 0 ? <Badge>Receivable</Badge> : u.currentBalance < 0 ? <Badge variant="destructive">Payable</Badge> : <Badge variant="secondary">Settled</Badge>}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={!!openUser} onOpenChange={(o) => !o && setOpenUser(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>{openUser?.name} — Transaction History</DialogTitle></DialogHeader>
          <div className="overflow-x-auto rounded-md border">
            <Table>
                ...
                {userTxs.length === 0 ? <TableRow><TableCell colSpan={5} className="text-center py-6 text-muted-foreground">No transactions</TableCell></TableRow>
                  : userTxs.map((t, idx) => (
                    <TableRow key={t.id} className={`row-tint-${idx % 4}`}>
                      ...
                      <TableCell>{t.category}</TableCell>
                      <TableCell className={`text-right font-medium ${t.type === "received" ? "text-green-600" : "text-red-600"}`}>{t.type === "received" ? "+" : "-"}{fmt(t.amount)}</TableCell>
